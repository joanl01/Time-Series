...
sent3b_binned_anom <- sent3b_binned_anom %>%
...
for (var in variables) {
  ...
  predictions <- as.numeric(sent3b_binned_anom[[var]])  # Ensure binary numeric format (0 and 1)
  true_labels <- as.numeric(sent3b_binned_anom$true_maneuver)  # Convert logical to numeric for consistency
...
plot(pr.curve(sent3b_binned_anom$`flag_Brouwer mean motion`,
              sent3b_binned_anom$true_maneuver, curve = TRUE),
