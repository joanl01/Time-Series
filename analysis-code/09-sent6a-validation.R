...
sent6a_anom <- inner_join(sent6a_res, sent6a_res_data, by = "date")
# Parameters
...
# Initialize the 'true_maneuver' column in 'sent6a_binned_anom' as FALSE
sent6a_binned_anom$true_maneuver <- numeric(nrow(sent6a_binned_anom))

# Loop through each maneuver date and each bin
for (i in 1:nrow(sent6a_tle_parsed_time)) {
  for (j in 1:(nrow(sent6a_binned_anom) - 1)) {  # Loop until the second-last row in 'sent6a_binned_anom'
    # Check if the maneuver date falls between the current bin and the next bin
    if (sent6a_tle_parsed_time$date[i] >= sent6a_binned_anom$date_bin[j] & sent6a_tle_parsed_time$date[i] < sent6a_binned_anom$date_bin[j + 1]) {
      sent6a_binned_anom$true_maneuver[j] <- 1
    }
  }
}
