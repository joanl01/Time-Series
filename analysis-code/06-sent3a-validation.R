...
sent3a_anom <- inner_join(sent3a_res, sent3a_res_data, by = "date")
# Parameters
...
# Initialize the 'true_maneuver' column in 'sent3a_binned_anom' as FALSE
sent3a_binned_anom$true_maneuver <- numeric(nrow(sent3a_binned_anom))

# Loop through each maneuver date and each bin
for (i in 1:nrow(sent3a_tle_parsed_time)) {
  for (j in 1:(nrow(sent3a_binned_anom) - 1)) {  # Loop until the second-last row in 'sent3a_binned_anom'
    # Check if the maneuver date falls between the current bin and the next bin
    if (sent3a_tle_parsed_time$date[i] >= sent3a_binned_anom$date_bin[j] & sent3a_tle_parsed_time$date[i] < sent3a_binned_anom$date_bin[j + 1]) {
      sent3a_binned_anom$true_maneuver[j] <- 1
    }
  }
}
